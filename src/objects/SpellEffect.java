...
public class SpellEffect
	{
		...
		private void applyEffect_84(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			if(num != 0)
			{
				...
				caster.addBuff(Constants.STATS_ADD_PA, num, 1, 0, true, spell,args,caster);
			}
		}
		...
		private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
		{
			if(turns <= 0)
			{
				...
				int finalDommage = Formulas.calculFinalDommage(fight,caster, caster,Constants.ELEMENT_NULL, dmg, false);
		...
		private void applyEffect_82(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_NULL, dmg,false);
		...
		private void applyEffect_91(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					...
					int dmg = Formulas.getRandomJet(args.split(";")[5]);
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_EAU, dmg,false);
		...
		private void applyEffect_92(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Formulas.getRandomJet(args.split(";")[5]);
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_TERRE, dmg,false);
		...
		private void applyEffect_93(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Formulas.getRandomJet(args.split(";")[5]);
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_AIR, dmg,false);
		...
		private void applyEffect_94(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Formulas.getRandomJet(args.split(";")[5]);
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_FEU, dmg,false);
		...
		private void applyEffect_95(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Formulas.getRandomJet(args.split(";")[5]);
					int finalDommage = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_NEUTRE, dmg,false);
		...
		private void applyEffect_108(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			{					
				...
				for(Fighter target : cibles)
				{
					int finalSoin = Formulas.calculFinalDommage(fight,caster, target,Constants.ELEMENT_FEU,  dmg, true);
